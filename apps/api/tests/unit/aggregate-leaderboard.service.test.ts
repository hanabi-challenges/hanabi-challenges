...
describe('computeAggregateRankings', () => {
  ...
  it('excludes players with no contributions', () => {
    const players = [makePlayer(1, [{ stageId: 1, score: 40 }]), makePlayer(2, [])];
    const result = computeAggregateRankings(players, { method: 'sum' });
  ...
  it('ranks players by total_score descending', () => {
    const players = [
      makePlayer(1, [{ stageId: 1, score: 30 }]),
      makePlayer(2, [{ stageId: 1, score: 50 }]),
    ];
    const result = computeAggregateRankings(players, { method: 'sum' });
  ...
  it('assigns same rank on ties', () => {
    const players = [
      makePlayer(1, [{ stageId: 1, score: 40 }]),
      makePlayer(2, [{ stageId: 1, score: 40 }]),
      makePlayer(3, [{ stageId: 1, score: 30 }]),
    ];
    const result = computeAggregateRankings(players, { method: 'sum' });
    expect(result[0].rank).toBe(1);
  ...
  it('best_n_of_m sums only top N scores', () => {
    ...
    const result = computeAggregateRankings(players, { method: 'best_n_of_m', n: 2 });
    expect(result[0].total_score).toBe(50); // 30 + 20, ignores 10
  });

  it('best_n_of_m: absent stage does not count (not zero)', () => {
    // Player 1 has scores 40 + 10 = 50, top 2 = 50
    // Player 2 has scores 30 + 30 = 60, top 2 = 60
    const players = [
      makePlayer(1, [
        { stageId: 1, score: 40 },
        { stageId: 2, score: 10 },
  ...
  it('best_n_of_m: n larger than contributions uses all', () => {
    const players = [makePlayer(1, [{ stageId: 1, score: 20 }])];
    const result = computeAggregateRankings(players, { method: 'best_n_of_m', n: 5 });
  ...
  it('rank_points converts rank to points from points_map', () => {
    ...
    const players = [
      makePlayer(1, [{ stageId: 1, score: 0, rank: 1 }]),
      makePlayer(2, [{ stageId: 1, score: 0, rank: 2 }]),
      makePlayer(3, [{ stageId: 1, score: 0, rank: 3 }]),
    ];
    const result = computeAggregateRankings(players, {
      method: 'rank_points',
  ...
  it('rank_points: rank beyond points_map gets 0', () => {
    const players = [makePlayer(1, [{ stageId: 1, score: 0, rank: 6 }])];
    const result = computeAggregateRankings(players, {
      method: 'rank_points',
      points_map: [10, 8, 6],
    });
  ...
  it('rank_points: null rank contributes 0', () => {
    const players = [makePlayer(1, [{ stageId: 1, score: 0, rank: null }])];
    const result = computeAggregateRankings(players, { method: 'rank_points', points_map: [10] });
    expect(result[0].total_score).toBe(0);
  });
  ...
  it('defaults to sum method when no method specified', () => {
    const players = [makePlayer(1, [{ stageId: 1, score: 25 }])];
    const result = computeAggregateRankings(players, {});
    expect(result[0].total_score).toBe(25);
  });
});

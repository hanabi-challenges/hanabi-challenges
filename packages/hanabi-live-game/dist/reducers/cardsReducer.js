...
exports.cardsReducer = cardsReducer;
...
const characters_1 = require("../rules/variants/characters");
const cardDeductionReducer_1 = require("./cardDeductionReducer");
const cardPossibilitiesReducer_1 = require("./cardPossibilitiesReducer");
const initialCardState_1 = require("./initialStates/initialCardState");
const reducerHelpers_1 = require("./reducerHelpers");
function cardsReducer(deck, action, game, metadata) {
    const variant = (0, gameData_1.getVariant)(metadata.options.variantName);
    const newDeck = [...deck];
    const hands = (0, complete_common_1.arrayCopyTwoDimensional)(game.hands);
    switch (action.type) {
        /**
        ...
        case "cardIdentity": {
            ...
            card = {
                ...card,
                revealedToPlayer: cardIdentityRevealedToPlayer(card, metadata.characterAssignments),
            };
            newDeck[order] = card;
            if (!(0, characters_1.shouldSeeSlot2CardIdentity)(metadata)) {
                break;
            }
            if (action.playerIndex === metadata.ourPlayerIndex
                || action.suitIndex === -1
                || action.rank === -1) {
        ...
        case "clue": {
            const clue = (0, clues_1.msgClueToClue)(action.clue, variant);
            const hand = game.hands[action.target];
            (0, complete_common_1.assertDefined)(hand, `Failed to find the hand at index: ${action.target}`);
            // eslint-disable-next-line func-style
            const applyClue = (order, positive) => {
                // Clues do not have to be applied in certain situations.
                if (!(0, clues_1.shouldApplyClue)(action.giver, metadata, variant)) {
                    return;
                }
                const card = getCard(newDeck, order);
                const newCard = (0, cardPossibilitiesReducer_1.cardPossibilitiesReducer)(card, clue, positive, metadata);
            ...
            for (const order of action.list) {
                ...
                newDeck[order] = {
                    ...
                    hasClueApplied: true,
                    firstCluedWhileOnChop: card.firstCluedWhileOnChop ?? (0, hand_1.isCardOnChop)(hand, deck, card),
                };
        ...
        case "play": {
            ...
            // inferred.
            const suitIndex = action.suitIndex === -1 ? card.suitIndex : action.suitIndex;
            ...
            const revealedToPlayer = action.suitIndex !== -1 && action.rank !== -1
                ? (0, complete_common_1.newArray)(constants_1.MAX_PLAYERS, true)
                : card.revealedToPlayer;
            const possibleCards = action.suitIndex !== -1 && action.rank !== -1
        ...
        case "draw": {
            ...
            if (game.turn.currentPlayerIndex !== action.playerIndex
            ...
            const initial = (0, initialCardState_1.getInitialCardState)(action.order, variant, metadata.options.numPlayers);
            ...
            const drawnCard = {
                ...
                revealedToPlayer: drawnCardRevealedToPlayer(action.playerIndex, game.hands.length, metadata.characterAssignments),
                possibleCards,
    ...
    if (game.turn.turnNum === 0
        && action.type === "draw"
        && !(0, deck_1.isInitialDealFinished)(newDeck.length, metadata)) {
        // No need to do deduction while cards are being drawn.
        return newDeck;
    }
    return (0, cardDeductionReducer_1.cardDeductionReducer)(newDeck, deck, action, hands, metadata);
}
// -------
// Helpers
// -------
function cardIdentityRevealedToPlayer(card, characterAssignments) {
    const revealedToPlayer = [];
    for (const playerIndex of (0, complete_common_1.tupleKeys)(characterAssignments)) {
        const characterName = (0, reducerHelpers_1.getCharacterNameForPlayer)(playerIndex, characterAssignments);
...
function drawnCardRevealedToPlayer(drawLocation, numPlayers, characterAssignments) {
    const revealedToPlayer = [];
    for (const i of (0, complete_common_1.eRange)(numPlayers)) {
        const playerIndex = i;
        revealedToPlayer.push(canPlayerSeeDrawnCard(playerIndex, drawLocation, numPlayers, characterAssignments));
    }
    return revealedToPlayer;
}
function canPlayerSeeDrawnCard(playerIndex, drawLocation, numPlayers, characterAssignments) {
...
function getCard(deck, order) {
    const card = deck[order];
    (0, complete_common_1.assertDefined)(card, `Failed to get the card in the deck at index: ${order}`);
    return card;
}
function revealCard(suitIndex, rank, card) {

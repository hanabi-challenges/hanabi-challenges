...
exports.sudokuIsCardNeededForMaxScore = sudokuIsCardNeededForMaxScore;
exports.sudokuWalkUpAll = sudokuWalkUpAll;
exports.sudokuGetMaxScorePerStack = sudokuGetMaxScorePerStack;
const complete_common_1 = require("complete-common");
const deck_1 = require("../deck");
/** Check if the card can still be played in a Sudoku variant. */
function sudokuIsCardNeededForMaxScore(suitIndex, rank, deck, playStackStarts, variant) {
    const allDiscardedSet = (0, deck_1.getAllDiscardedSetForSuit)(variant, deck, suitIndex);
    const { maxScoresForEachStartingValueOfSuit } = sudokuWalkUpAll(allDiscardedSet, variant);
    const playStackStart = playStackStarts[suitIndex];
    (0, complete_common_1.assertDefined)(playStackStart, `Failed to find the play stack start for suit index: ${suitIndex}`);
    const possibleStackStartRanks = playStackStart === null
    ...
    return possibleStackStartRanks.some((possibleStackStartRank) => {
        const longestSequence = (rank - possibleStackStartRank + variant.stackSize) % variant.stackSize;
        const score = maxScoresForEachStartingValueOfSuit[possibleStackStartRank - 1];
        (0, complete_common_1.assertDefined)(score, `Failed to find the max score for starting suit index ${suitIndex} at start rank: ${possibleStackStartRank}`);
 ...
 */
function sudokuWalkUpAll(allDiscardedSet, variant) {
    const maxScoresForEachStartingValueOfSuit = (0, complete_common_1.newArray)(variant.suits.length, variant.stackSize);
    let lastDead = 0;
    for (const currentRank of variant.ranks) {
        if (allDiscardedSet.has(currentRank)) {
            // We hit a new dead rank.
            for (const writeRank of (0, complete_common_1.eRange)(lastDead + 1, currentRank)) {
    ...
    if (lastDead !== 5) {
        ...
        // to them.
        for (const writeRank of (0, complete_common_1.iRange)(lastDead + 1, variant.stackSize)) {
            ...
            const scoreWithWraparound = scoreForStartingAt1 + scoreForPlayingUpToLargestRank;
 ...
 */
function sudokuGetUnusedStackStartRanks(playStackStarts, variant) {
    return variant.ranks.filter((rank) => !playStackStarts.includes(rank));
}
function incrementFiveStackIndex(i) {
    const val = i + 1;
    assertFiveStackIndex(val);
    return val;
}
function decrementFiveStackIndex(i) {
    const val = i - 1;
    assertFiveStackIndex(val);
    return val;
}
function assertFiveStackIndex(val) {
    if (val < 0 || val > 4) {
        throw new TypeError(`A FiveStackIndex was ${val}, but it must be between 0 and 4.`);
    }
}
/**
...
function evaluateAssignment(curAssignment, unassignedSuits, possibleStackStarts, maxPartialScores) {
    ...
    for (const [assignedLocalSuitIndex, assignedStackStartIndex,] of curAssignment.entries()) {
        ...
        (0, complete_common_1.assertDefined)(assignedSuit, `Unexpected assigned local suit index ${assignedLocalSuitIndex} encountered while evaluating assignment.`);
        const assignedStackStart = possibleStackStarts[assignedStackStartIndex];
        (0, complete_common_1.assertDefined)(assignedStackStart, "Failed to retrieve the stack start while solving the assignment problem since the index access was out of range.");
        const maxPartialScoresForThisSuit = maxPartialScores[assignedSuit];
        (0, complete_common_1.assertDefined)(maxPartialScoresForThisSuit, `Failed to retrieve the max partial scores for suit: ${assignedSuit}`);
        ...
        (0, complete_common_1.assertDefined)(value, `Failed to retrieve the max score for starting suit ${assignedSuit} at rank ${assignedStackStart}.`);
 ...
 */
function isAssignmentBetter(assignmentValue, assignmentSorted, bestAssignmentSum, bestAssignmentSorted) {
    ...
    if (assignmentValue === bestAssignmentSum) {
        ...
        for (const [i, val] of assignmentSorted.entries()) {
            ...
            (0, complete_common_1.assertDefined)(valBestAssignment, "Failed to retrieve the currently best stored assignment entry.");
...
function findNextAssignment(curAssignedStackStartIndex, possibleStackStarts, assignedStackStarts) {
    if (curAssignedStackStartIndex !== 4) {
        // If the assignment was undefined before, we start at 0, otherwise start at the next value.
        const firstStackStartToTry = curAssignedStackStartIndex === undefined
            ? 0
            : incrementFiveStackIndex(curAssignedStackStartIndex);
        for (const nextAssignment of eRange5(firstStackStartToTry, possibleStackStarts.length)) {
 ...
 * extra-round.
 */
function sudokuGetMaxScorePerStack(deck, playStackStarts, variant) {
    const independentPartOfMaxScore = [];
    const maxPartialScores = [];
    const unassignedSuits = [];
    // Find the suits for which we need to solve the assignment problem.
    for (const [i, stackStart] of playStackStarts.entries()) {
        const suitIndex = i;
        const allDiscardedSet = (0, deck_1.getAllDiscardedSetForSuit)(variant, deck, suitIndex);
        const { allMax, maxScoresForEachStartingValueOfSuit } = sudokuWalkUpAll(allDiscardedSet, variant);
        if (allMax) {
        ...
        if (stackStart !== null) {
            ...
            (0, complete_common_1.assertDefined)(score, `Failed to find the max score for the starting suit index ${suitIndex} at start: ${stackStart}`);
    ...
    // stack start.
    const assigned = [false, false, false, false, false];
    ...
    while (true) {
        ...
        const nextAssignment = findNextAssignment(curAssignedStackStartIndex, possibleStackStarts, assigned);
        if (nextAssignment !== undefined) {
            ...
            if (localSuitIndex === unassignedSuits.length - 1) {
                const { assignmentValue, assignment } = evaluateAssignment(curAssignment, unassignedSuits, possibleStackStarts, maxPartialScores);
                const assignmentSorted = [...assignment];
                assignmentSorted.sort((a, b) => a - b);
                if (isAssignmentBetter(assignmentValue, assignmentSorted, bestAssignmentSum, bestAssignmentSorted)) {
            ...
            if (localSuitIndex < unassignedSuits.length - 1) {
                ...
                for (const higherLocalSuitIndex of eRange5(incrementFiveStackIndex(localSuitIndex), unassignedSuits.length)) {

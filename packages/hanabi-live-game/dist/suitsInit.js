...
exports.suitsInit = suitsInit;
const tslib_1 = require("tslib");
const complete_common_1 = require("complete-common");
const abbreviations_1 = require("./abbreviations");
const constants_1 = require("./constants");
const suits_json_1 = tslib_1.__importDefault(require("./json/suits.json"));
function suitsInit(COLORS) {
    const suits = new Map();
    if (suits_json_1.default.length === 0) {
        throw new Error('The "suits.json" file did not have any elements in it.');
    }
    for (const suitJSON of suits_json_1.default) {
        ...
        if (suitJSON.pip === "" && suitJSON.name !== "Unknown") {
            throw new Error(`The "pip" property for the suit "${suitJSON.name}" is empty.`);
        }
        const { pip } = suitJSON;
        ...
        if (abbreviation.length !== 1) {
            throw new Error(`The "${suitJSON.name}" suit has an abbreviation of "${abbreviation}" that is not one letter long.`);
        }
        if (abbreviation !== abbreviation.toUpperCase()) {
            throw new Error(`The "${suitJSON.name}" suit has an abbreviation of "${abbreviation}" that is not an uppercase letter.`);
        }
        if (abbreviations_1.ALL_RESERVED_NOTES.has(abbreviation.toLowerCase())) {
            throw new Error(`The "${suitJSON.name}" suit has an abbreviation of "${abbreviation}" that conflicts with a reserved note word.`);
        }
        // Validate the "createVariants" property.
        if (suitJSON.createVariants === false) {
            throw new Error(`The "createVariants" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const createVariants = suitJSON.createVariants ?? false;
        // Validate the "displayName" property.
        if (suitJSON.displayName === "") {
            throw new Error('There is a suit with an empty display name in the "suits.json" file.');
        }
        ...
        // Validate the fill.
        const { fill, fillColorblind } = getSuitFillAndFillColorblind(suitJSON, COLORS, clueColors);
        // Validate the fill colors.
        if (suitJSON.fillColors !== undefined && suitJSON.fillColors.length === 0) {
            throw new Error(`The "fillColors" array for the suit "${suitJSON.name}" is empty.`);
        }
        ...
        if (suitJSON.oneOfEach === false) {
            throw new Error(`The "oneOfEach" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const oneOfEach = suitJSON.oneOfEach ?? false;
        if (suitJSON.allClueColors === false) {
            throw new Error(`The "allClueColors" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const allClueColors = suitJSON.allClueColors ?? false;
        if (suitJSON.noClueColors === false) {
            throw new Error(`The "noClueColors" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const noClueColors = suitJSON.noClueColors ?? false;
        if (suitJSON.allClueRanks === false) {
            throw new Error(`The "allClueRanks" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const allClueRanks = suitJSON.allClueRanks ?? false;
        if (suitJSON.noClueRanks === false) {
            throw new Error(`The "noClueRanks" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const noClueRanks = suitJSON.noClueRanks ?? false;
        if (suitJSON.prism === false) {
            throw new Error(`The "prism" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        const prism = suitJSON.prism ?? false;
        if (suitJSON.inverted === false) {
            throw new Error(`The "inverted" property for the suit "${suitJSON.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
        }
        ...
        suits.set(suitJSON.name + constants_1.SUIT_REVERSED_SUFFIX, suitReversed);
    }
    return suits;
}
function getSuitClueColors(suitJSON, COLORS) {
    if (suitJSON.clueColors !== undefined) {
        ...
        return suitJSON.clueColors.map((colorName) => {
            ...
            (0, complete_common_1.assertDefined)(color, `The clue color "${colorName}" for the suit "${suitJSON.name}" does not exist.`);
            return color;
        });
    }
    ...
    if (suitJSON.allClueColors === true
    ...
    throw new Error(`Failed to derive the clue colors for the "${suitJSON.name}" suit. (There is no corresponding color named "${suitJSON.name}".)`);
}
/**
 ...
 */
function getSuitFillAndFillColorblind(suitJSON, COLORS, clueColors) {
    if (suitJSON.fill === "") {
        throw new Error(`The fill property was empty for the "${suitJSON.name}" suit. Perhaps it should be removed entirely from the "suits.json" file?`);
    }
    ...
    throw new Error(`Failed to find the fill for the "${suitJSON.name}" suit. (There is no corresponding color named "${suitJSON.name}" and this suit has no clue colors specified.)`);
}
//# sourceMappingURL=suitsInit.js.map
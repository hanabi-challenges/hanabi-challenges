...
exports.variantsInit = variantsInit;
exports.createVariant = createVariant;
const tslib_1 = require("tslib");
const complete_common_1 = require("complete-common");
const abbreviations_1 = require("./abbreviations");
const constants_1 = require("./constants");
const variants_json_1 = tslib_1.__importDefault(require("./json/variants.json"));
const notes_1 = require("./notes");
const RankClueNumber_1 = require("./types/RankClueNumber");
function variantsInit(colorsMap, suitsMap) {
    const variants = new Map();
    if (variants_json_1.default.length === 0) {
        throw new Error('The "variants.json" file did not have any elements in it.');
    }
    // Fields are validated in the order that they appear in "VariantDescription.ts".
    for (const variantJSON of variants_json_1.default) {
        // ------------------------
        ...
        if (variantJSON.newID === "") {
            throw new Error(`The "${variantJSON.name}" variant has a blank "newID" property.`);
        }
...
function createVariant(colorsMap, suitsMap, variantDescription, id, newID) {
    // ---------------
    // Core properties
    // ---------------
    // Validate the name.
    ...
    const suits = variantDescription.suits.map((suitName) => {
        ...
        (0, complete_common_1.assertDefined)(suit, `The "${name}" variant has a "${suitName}" suit, which does not exist.`);
        return suit;
    });
    ...
    if (variantDescription.clueRanks !== undefined) {
        for (const clueRank of variantDescription.clueRanks) {
            if (!(0, RankClueNumber_1.isValidRankClueNumber)(clueRank)) {
                throw new Error(`The "clueRanks" property for the variant "${name}" has an invalid value of: ${clueRank}`);
            }
        }
    }
    const clueRanks = variantDescription.clueRanks ?? [...constants_1.DEFAULT_CLUE_RANKS];
    // Validate the "stackSize" property.
    if (variantDescription.stackSize === constants_1.DEFAULT_FINISHED_STACK_LENGTH) {
        throw new Error(`The "stackSize" property for the variant "${variantDescription.name}" must not be set to ${constants_1.DEFAULT_FINISHED_STACK_LENGTH}. If it is intended to be ${constants_1.DEFAULT_FINISHED_STACK_LENGTH}, then remove the property altogether.`);
    }
    const stackSize = variantDescription.stackSize ?? constants_1.DEFAULT_FINISHED_STACK_LENGTH;
    // -----------------------------------------------
    ...
    if (variantDescription.clueStarved === false) {
        throw new Error(`The "clueStarved" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.colorCluesTouchNothing === false) {
        throw new Error(`The "colorCluesTouchNothing" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    const colorCluesTouchNothing = variantDescription.colorCluesTouchNothing ?? false;
    ...
    if (variantDescription.rankCluesTouchNothing === false) {
        throw new Error(`The "rankCluesTouchNothing" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    const rankCluesTouchNothing = variantDescription.rankCluesTouchNothing ?? false;
    // Validate the "alternatingClues" property. If it is not specified, assume false.
    if (variantDescription.alternatingClues === false) {
        throw new Error(`The "alternatingClues" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.cowAndPig === false) {
        throw new Error(`The "cowAndPig" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.duck === false) {
        throw new Error(`The "duck" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.oddsAndEvens === false) {
        throw new Error(`The "oddsAndEvens" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.synesthesia === false) {
        throw new Error(`The "synesthesia" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.upOrDown === false) {
        throw new Error(`The "upOrDown" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.throwItInAHole === false) {
        throw new Error(`The "throwItInAHole" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.funnels === false) {
        throw new Error(`The "funnels" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.chimneys === false) {
        throw new Error(`The "chimneys" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.sudoku === false) {
        throw new Error(`The "sudoku" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    const ranks = [...constants_1.DEFAULT_CARD_RANKS].slice(0, stackSize);
    if (upOrDown) {
        // The "Up or Down" variants have START cards.
        ranks.push(constants_1.START_CARD_RANK);
    }
    // --------------------------------------------
    ...
    if (variantDescription.specialRank !== undefined
        ...
        throw new Error(`The "specialRank" property for the variant "${variantDescription.name}" is invalid: ${variantDescription.specialRank}`);
    }
    ...
    if (variantDescription.specialRankAllClueColors === false) {
        throw new Error(`The "specialRankAllClueColors" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    const specialRankAllClueColors = variantDescription.specialRankAllClueColors ?? false;
    ...
    if (variantDescription.specialRankAllClueRanks === false) {
        throw new Error(`The "specialRankAllClueRanks" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    const specialRankAllClueRanks = variantDescription.specialRankAllClueRanks ?? false;
    ...
    if (variantDescription.specialRankNoClueColors === false) {
        throw new Error(`The "specialRankNoClueColors" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    const specialRankNoClueColors = variantDescription.specialRankNoClueColors ?? false;
    ...
    if (variantDescription.specialRankNoClueRanks === false) {
        throw new Error(`The "specialRankNoClueRanks" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    const specialRankNoClueRanks = variantDescription.specialRankNoClueRanks ?? false;
    ...
    if (variantDescription.specialRankDeceptive === false) {
        throw new Error(`The "specialRankDeceptive" property for the variant "${variantDescription.name}" must be set to true. If it is intended to be false, then remove the property altogether.`);
    }
    ...
    if (variantDescription.criticalRank !== undefined
        ...
        throw new Error(`The "criticalRank" property for the variant "${variantDescription.name}" is invalid: ${variantDescription.criticalRank}`);
    }
    ...
    const offsetCornerElements = suits.some((suit) => suit.clueColors.length > 1);
    const suitAbbreviations = (0, abbreviations_1.getUppercaseSuitAbbreviationsForVariant)(name, suits);
    ...
    const identityNotePattern = (0, notes_1.getIdentityNotePatternForVariant)(suits, ranks, suitAbbreviations, isUpOrDown);
    const hasInverted = suits.some((suit) => suit.inverted);
    // Add it to the map.
    const variant = {
        name,
        suits,
...
function getVariantClueColors(variantDescription, COLORS, suits) {
    ...
    return variantDescription.clueColors.map((colorString) => {
        ...
        (0, complete_common_1.assertDefined)(color, `The "${variantDescription.name}" variant has a clue color of "${colorString}", which does not exist.`);
        return color;
    });
}
function getVariantShowSuitNames(suits, upOrDown, sudoku) {
    ...
    const hasSuitIDThatEndsWithANumber = suits.some((suit) => {
        ...
        (0, complete_common_1.assertDefined)(finalIDCharacter, `Failed to get the final character of the ID of suit: ${suit.name}`);
        const finalIDNumber = (0, complete_common_1.parseIntSafe)(finalIDCharacter);
    ...
    const hasSuitThatIsTouchedByTwoOrMoreClueColors = suits.some((suit) => suit.clueColors.length >= 2);
    ...
    // easily identifiable.
    const variantHasReversedSuits = suits.some((suit) => suit.reversed);
...
//# sourceMappingURL=variantsInit.js.map